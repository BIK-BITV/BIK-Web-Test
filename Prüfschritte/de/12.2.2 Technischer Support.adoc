...
können, die die Web-App betrifft.
Dies soll mindestens die Informationen umfassen, die dazu in der
Web-App-Dokumentation erwähnt werden (siehe auch Prüfschritt
...
vollständig geklärt.
Hinweise dazu können Sie in einem Git.
Hub {web-issue-url}[Issue hinterlassen].

Zielführend könnte es bspw. sein, anhand der Informationen aus der
Web-App-Dokumentation, den technischen Support zu kontaktieren und einige
