...
angemessener Weise ersetzt (siehe
<<2.6 Angemessene Alternativtexte>>).
...
sichtbaren Text (Stand-alone-Icon), so sollte eine Textalternative vorhanden
sein.
...
<<2.6 Angemessene Alternativtexte>>)Das `title-` bzw. ``desc-``Element sollte
...
Die Frage ist daher, was im Alternativtext sinnvoll herübergebracht werden kann.
