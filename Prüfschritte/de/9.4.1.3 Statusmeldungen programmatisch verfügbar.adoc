...
Ergebnis von Interaktionen (wie z. B. die Anzahl der beim Filtern einer
...
. Eingaben vornehmen, die zur Generierung von Statusmeldungen führen.
  Wenn das
  Angebot von sich aus Statusmeldungen, z. B. bei aktualisierten Inhalten,
...
Statusmeldung in ein bereits bestehendes Element eingefügt wird oder ob eine
