...
. Prüfen, ob Inhalte, die visuell als Überschriften zu erkennen sind, mittels der HTML-Strukturelemente `h1` bis `h6` als Überschriften ausgezeichnet sind. Die Hierarchie der eingesetzten HTML-Überschriftselemente soll der Gliederung der Inhalte entsprechen.
...
* Die erste Regel für den Einsatz von WAI-ARIA besagt, man soll wenn möglich native HTML-Elemente einsetzen. Siehe https://www.w3.org/TR/using-aria/[Using of ARIA, First rule of ARIA use] (W3C). Wenn dies aus irgendeinem Grund nicht möglich ist, kann mit `role="heading"` und dem `aria-level`-Attribut die Semantik zur Verfügung gestellt werden. Dies sollte im Zweifelsfall geprüft werden.
...
. Ist das Verzeichnis vollständig? Zeigt es mir alle wichtigen Inhalte der Seite?
. Ist das Verzeichnis brauchbar? Kann ich ohne viel Aufwand auf alle wichtigen Inhalte der Seite zugreifen?
