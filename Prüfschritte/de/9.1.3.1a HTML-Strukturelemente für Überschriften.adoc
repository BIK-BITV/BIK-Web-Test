...
* Die erste Regel für den Einsatz von WAI-ARIA besagt, man soll wenn möglich native HTML-Elemente einsetzen. Siehe https://www.w3.org/TR/using-aria/[Using of ARIA, First rule of ARIA use] (W3C)Wenn dies nicht möglich ist, kann mit `role="heading"` und dem ``aria-level``-Attribut die Semantik zur Verfügung gestellt werden.
...
* Es wid semantisches Mark-Up für Überschriften eingesetzt, die Verschachtelung der Hierarchie-Ebenen ist aber irreführend, entspricht nicht der inhaltlichen Gliederung.
