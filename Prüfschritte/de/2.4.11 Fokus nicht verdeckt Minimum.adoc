...
(z. B. Verwendung der Esc-Taste, um den geöffneten Inhalt zu schließen, Verwendung von Tasten zum Scrollen des Inhalts oder Betätigung einer Taste, um zwischen Überlagerungen zu wechseln).
...
*	Cookie-Dialoge werden häufig als *nicht-modale Cookie-Dialoge* umgesetzt und führen zu Problemen im Sinne des Prüfschritts. Ein Lösungsansatz wäre, den Cookie-Hinweis statt als nicht-modalen Dialog als Teil der Seite umzusetzen. D.h. die Cookie-Meldung verschiebt den Content (statt ihn zu überlagern), solange die Meldung eingeblendet ist.
...
*	Wenn Inhalte von Nutzenden neu positioniert werden können, werden nur die Anfangspositionen für die Prüfung berücksichtigt.
