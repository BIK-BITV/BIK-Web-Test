...
** Dazu im Inspektor des Browsers das `track`-Element suchen. Die Untertitel-Datei ist als .vtt oder .scrt-Datei über das `track`-Element eingebunden. 
...
** Alternativ den Pfad kopieren. Den Pfad im Browser hinter dem / der toplevel Domain eingeben.
...
Auch wenn Fehler in der Zeitcodierung der Track-Datei gefunden werden, ist davon auszuegehen, dass der Fehler nicht beim Player liegt. Es ist es jedoch nicht ganz auszuschließen, dass eine nicht vorhandene Synchronität an der Untertitel-Datei _und_ am Player liegt.
