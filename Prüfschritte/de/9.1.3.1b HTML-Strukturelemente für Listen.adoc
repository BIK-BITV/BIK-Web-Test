...
"Definitionslisten (`dl`) repräsentieren eine Liste von Gruppen von Begriffen (spezifiziert mit dem `dt`-Element) 
und Beschreibungen (bereitgestellt durch `dd`-Elemente). 
Typische Anwendungen für das Konstrukt sind Glossare oder die Darstellung von Schlüssel/Wert-Paaren.
Ihr Einsatz für andere gemeinhin übliche paarige Informationen wird akzeptiert.
