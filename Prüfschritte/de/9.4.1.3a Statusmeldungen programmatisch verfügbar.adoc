= Prüfschritt 9.4.1.3a Statusmeldungen programmatisch verfügbar
...
Statusmeldungen der Inhalte sind über Rollen oder Eigenschaften programmatisch
ermittelbar, so dass sie von Hilfsmitteltechnologie ausgegeben werden können,
ohne den Fokus zu erhalten.

== Was wird geprüft?

Wenn Webanwendungen Statusmeldungen erzeugen, etwa wenn ein Produkt in einen
digitalen Warenkorb gelegt wurde, ein Formuar beim Abschicken eine
Fehlermeldung oder ein Bestätigungsmeldung erzeugt, sollen visuell
...
Abschicken eines Formulars die Seite neulädt und dann vor dem Formular eine
Fehlermeldung erscheint.

=== 2Prüfung

Screenreader NVDA im Firefox-Browser aktivieren.
Eingaben vornehmen, die zur Generierung von Statusmeldungen führenWenn das
Angebot von sich aus Statusmeldungen, z.Bbei aktualisierten Inhalten,
...
Screenreader unterschiedlich ausfallenDer Erfolg kann davon abhängen, ob die
Statusmeldung in ein bereits bestehendes Elementes eingefügt wird oder ob ein
kurze Zeitverzögerung vor der Generierung der Meldung definiert worden ist.
...
Statusmeldungen in der Regel nicht eingesetzt werdenDie Angabe der geeigneten
Rolle definiert passende Werte für diese Attribute bereits implizit.
