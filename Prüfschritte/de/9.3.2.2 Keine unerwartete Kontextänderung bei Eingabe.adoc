...
Eingaben von Nutzenden bei Formularelementen sollen nicht zu unerwarteten Kontext-Änderungen führen, außer Nutzende werden vor der Verwendung der Komponente auf dieses Verhalten hingewiesen. Erfolgen Änderungen unterhalb der Formulareingaben (z. B. neue Felder werden eingeblendet) und die Gesamtstruktur bleibt gleich, so bleibt auch der Kontext für die Nutzenden erhalten und es handelt sich nicht um eine unerwartete Kontextänderung. Der Fokus soll aufgrund der Eingaben nicht versetzt werden.

== Warum wird das geprüft?

Unerwartete und unangekündigte Kontextänderungen bei Formulareingaben oder einer Auswahl einer Option können die Orientierung von Nutzern beeinträchtigen, z.B. die Auswahl einer Checkbox oder ein Radio-Inputs ruft eine neue Seite auf, eine Auswahlliste ohne Submit-Button reagiert auf `onchange`. Kontextänderungen auf der Seite selbst können Nutzende ablenken oder auch unbemerkt bleiben und dadurch für Verwirrung sorgen (plötzlich sind Inhalte verändert). Sie sollten deshalb erwartet und klar nachvollziehbar sein.
...
Prüfen Sie, ob die Eingabe von Daten oder die Änderung von deren Einstellungen einen unerwarteten Kontextwechsel (siehe Erläuterung unter "3. Hinweise") auslöst. 

. Seite im Browser (Chrome oder Firefox) aufrufen.
. Mit der Tastatur zu allen Formularelementen navigieren, in die Nutzende Daten eingeben oder eine Einstellung des Elements ändern können.
. Prüfen, ob die Eingabe (z. B. Eingabe von Text in ein Textfeld, Aktivierung einer Checkbox oder Radio-Inputs) oder Änderung von Einstellungen (z.B. Option eines Selects auswählen) eine Kontextänderung erzeugt.
. Prüfen, ob Inhaltsänderungen (etwa Einblendungen zusätzlicher Formularfelder) begrenzt sind, die Änderung nach dem Formularelement erfolgt und der Kontext erhalten bleibt. In diesem Fall handelt es sich nicht um eine unerwartete Kontextänderung.
. Prüfen, ob durch Formulareingaben hervorgerufene Kontextänderungen (etwa dynamisch eingeblendete Elemente) den aktuellen Fokus versetzen. Dies gilt als unerwartete Kontextänderung.
. Prüfen, ob der Kontext geändert wird, wenn das letzte Feld eines Formulars ausgefüllt wird. Das Formular soll dadurch nicht abgeschickt werden.
. Gibt es ein Kontextwechsel, prüfen, ob der Kontextwechsel mit Hilfe von sichtbaren, zugänglichen Erläuterungen angekündigt wird. 

=== 3. Hinweise

* Fehlermeldungen, die über die JavaScript-Funktion `alert()` ausgegeben werden, öffnen sich als Antwort auf Eingaben von Nutzenden und sie sind modal, in Hinblick auf die Orientierung daher unproblematisch. Sie gelten nicht als unerwartete Kontextänderungen im Sinne des Prüfschritts.
