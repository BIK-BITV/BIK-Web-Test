...
z.B. Sprach- und Videotelefonie), muss sie ebenfalls Textkommunikation in Echtzeit zur Verfügung 
stellen. Echtzeit bedeutet hier, dass schon die Eingabe einzelner Zeichen übertragen und dem 
Empfänger angezeigt wird. Die Nutzenden müssen den eingegebenen Text nicht erst aktiv abschicken.

== Warum wird das geprüft?

Menschen, die nicht Sprache nutzen können oder wollen, sollen die Möglichkeit haben, 
über Echtzeit-Textkommunikation (RTT, Real-time Text) gleichrangig an Kommunikation teilzuhaben.
...
. Webangebot öffnen.
. Wenn eine Funktion für die Zwei-Wege-Kommunikation geboten wird, prüfen ob auch Textkommunikation in Echtzeit unterstützt wird.
