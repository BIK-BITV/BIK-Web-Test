...
. In der _Web Developer Toolbar_ das Menü _Images > Display Alt Attributes_
  ...
  Alternativtexten versehen sind (siehe link:#angemessen[2.4])Alternativ kann
  auch das
...
. Feststellen, ob die Seite Image maps enthält.
  ...
  Ist ein Image map vorhanden, weiter mit 2.2.
...
==== 2.3Textalternativen für Hintergrundgrafiken
...
Bild in angemessener Weise ersetzt (siehe link:#angemessen[2.6]).
...
==== 2.6Gleichwertige (äquivalente) Alternativtexte:
...
* [#G92][#H30][#F20]Bei *verlinkten Abbildungen* gibt es folgende
  Möglichkeiten:
...
Denn gängige Screenreader verlassen sich nicht darauf, daß leere
...
==== 3.2Unterstützung von SVGs durch assistive Technologien
...
Auch wenn Alternativtexte im Bereich der
...
Viele Teaser-Bilder sind vom Inhalt her ohnehin nicht
...
Kontext dagegen normalerweise nicht klar.
Er müsste also tatsächlich Vermutungen
...
Nach wie vor werden von JAWS
die Inhalte von ``alt``-Attribut und ``title``-Attribut nur alternativ
vorgelesen.
...
Sehr häufig bedeutet das: der
Alternativtext sagt, was abgebildet ist.

Das ``title``-Attribut ist dagegen für _ergänzende_ Informationen zum Bild
vorgesehen.
Es kann zum Beispiel verwendet werden, um zu sagen, von welcher
Quelle das Bild stammt.
...
Der Benutzer sieht nicht genau, was
abgebildet ist oder er versteht nicht, wofür das Bild stehen soll.
Für diesen Besucher ist eine ergänzende Beschriftung nützlich.
Sie steht am besten als Text neben dem Bild, denn dann ist sie auch für
Tastaturnutzer sichtbar.
Aber auch das ``title``-Attribut kann verwendet werden, denn die Beschriftung ist (für den
...
nicht ein positives Ergebnis des BITV-Tests.
Der Webauftritt wird positiv bewertet, wenn er das Seine zur Sicherung der
Zugänglichkeit beisteuert.
Das Hilfsmittel ist dagegen gut, wenn es auch mit schlechten Webauftritten klar
kommt.
...
Denn Screenreader oder Textbrowser stellen den
...
liefert der Screenreader diese
...
Das Logo ist aber auch selbst wichtig, es soll dem
...
In diesem Fall kommt es nur (oder jedenfalls
hauptsächlich) auf die Vermittlung des Linkziels an.
...
Auch für einen blinden Benutzer ist es gut, zu wissen, dass die Firma ihr
Logo auf der Seite zeigt.
