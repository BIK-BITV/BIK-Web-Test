...
* durch zusätzliches `title`-Attribut auf dem Link
...
* Das ``title``-Attribut eines Links ist potenziell programmatisch ermittelbar, wird aber in manchen Fällen (abhängig von Einstellungen) nicht von Screenreadern ausgewertet und ist auch für Tastaturnutzende und bei der Touch-Bedienung nicht verfügbar.
...
Bei verlinkten Grafiken ist der Linktext der ``alt``-Text des ``img``-Elements. Die Aussagekraft eines vorhandenen ``alt``-Textes wird jedoch im Prüfschritt <<9.2.4.6 Aussagekräftige Überschriften und Beschriftungen.adoc#,2.4.6 Aussagekräftige Überschriften und Beschriftungen>>
...
endif::env_embedded[] oder 
geprüft.

In diesem Prüfschritt wird nur geprüft, ob die Information über das Dateiformat überhaupt vorhanden ist.
Das ist der Fall, wenn das Dateiformat in einer Grafik, im Linktext oder im ``title``-Attribut des Links angegeben wird.

Ist die Grafik, die Auskunft über das Dateiformat gibt, nicht im Link eingebunden (z. B. unverlinkt dem Link vorangestellt), so wird das Vorhandensein von Alternativtext im Prüfschritt 
...
endif::env_embedded[] und dessen Aussagekraft im Prüfschritt <<9.2.4.6 Aussagekräftige Überschriften und Beschriftungen.adoc#,2.4.6 Aussagekräftige Überschriften und Beschriftungen>>
geprüft.
