...
. In der _Web Developer Toolbar_ das Menü _Images > Display Alt Attributes_ aufrufen. Alternativ kann auch das https://www.bitvtest.de/bitv_test/das_testverfahren_im_detail/werkzeugliste.html[Images bookmarklet] eingesetzt werden.
. Prüfen, ob Layoutgrafiken und dekorative Grafiken leere `alt`-Attribute enthalten. Falls bei solchen Grafiken `title`-Attribute vergeben sind, müssen auch diese leer sein.
. [#F38]####Zu bemängeln sind komplett fehlende `alt`-Attribute (werden durch die Toolbar-Funktion mit "NoAlt!" gekennzeichnet) und auch Bezeichnungen wie "Abstandshalter", "spacer", "leer" etc. für Layoutgrafiken.
