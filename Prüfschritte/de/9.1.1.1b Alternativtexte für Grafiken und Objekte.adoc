...
. Seite in
  https://www.bitvtest.de/bitv_test/das_testverfahren_im_detail/werkzeugliste.html#firefox[
  Firefox] aufrufen.
. Mit den Web Developer Tools des Browsers prüfen, ob das Video im  HTML-Code über ein `video`-Tag oder über ein `iframe`-Tag eingebunden
  wurde.
. Bei einem *Video-Tags* kann die Textaltenative mittels der ARIA Attribute
  `aria-label` oder `aria-labelledby` angegeben werden, , wenn das `controls`-Attribut vorhanden ist.
  Wird das Video innerhalb eines `figure` -Tag eingebunden, kann auch ein 
  `figcaption`-tag genutzt werden.
. Bei einer Einbindung über *iframe* (z.B. Youtube) erfolgt die Angabe einer Textalternative mittels `title`-Attribut.
