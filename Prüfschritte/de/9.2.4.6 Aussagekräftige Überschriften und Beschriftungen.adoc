...
Visuell werden Webseiten-Inhalte durch Überschriften strukturiert. Dank dieser Strukturierung weiß der Benutzer, was zusammengehört, kann die
Inhalte der Webseite leicht überblicken und gezielt auf Inhalte zugreifen, die ihn interessieren.

Wenn Formularelemente sinngebend beschriftet sind, können Sie von Nutzer als solche erkannt und bedient werden.
...
. Wenn zur Beschriftung `fieldset` mit `legend` eingesetzt wird: Sind `legend` und ``label``, zusammen vorgelesen, bei allen Formularelementen im Fieldset
  sinnvoll?
