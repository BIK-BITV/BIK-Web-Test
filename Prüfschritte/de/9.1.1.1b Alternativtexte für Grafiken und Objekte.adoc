...
. Prüfen, ob die Seite Objekte enthält. 
  ...
  In der https://www.bitvtest.de/bitv_test/das_testverfahren_im_detail/werkzeugliste.html#webdeveloper[
  Web Developer Toolbar] _CSS > Disable All Styles_ und _Images >
  Hide Images_ wählen.
...
. Seite in
  https://www.bitvtest.de/bitv_test/das_testverfahren_im_detail/werkzeugliste.html#firefox[
  Firefox] aufrufen.
. Mit den Web Developer Tools des Browsers prüfen, ob mit der
...
. Seite in
  https://www.bitvtest.de/bitv_test/das_testverfahren_im_detail/werkzeugliste.html#firefox[
  Firefox] aufrufen.
. Mit den Web Developer Tools des Browsers prüfen, ob es sich um eine
...
==== 2.5 Textalternativen für Videos

. Seite in
  https://www.bitvtest.de/bitv_test/das_testverfahren_im_detail/werkzeugliste.html#firefox[
  Firefox] aufrufen.
. Mit den Web Developer Tools des Browsers prüfen, ob das Video im 
  HTML-Code über ein `video`-Tag oder über ein `iframe`-Tag eingebunden
  wurde.
. Bei einem *Video-Tags* kann die Textaltenative mittels der ARIA Attribute
  `aria-label, aria-labelledby, and aria-describedby` angegeben werden.
  Wird das Video innerhalb eines `figure` -Tag eingebunden kann auch ein 
  `figcaption`-tag genutzt werden.
. Bei einer Einbindung über *iframe* (z.B. Youtube) erfolgt die Angabe 
  einer Textalternative mittels `title`-attribute.
...
* entsprechend beschriftete Schaltfläche, z.B. `aria-label="Video 
  [Titel des Videos] abspielen"`
