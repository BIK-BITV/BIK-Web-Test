...
* Ton wieder anschalten oder Seite neu laden, dann die Lautstärke ganz herunterregeln.
...
Sprache oder laute Geräusche etwa stören mehr als eine gleichmäßige Hintergrundmusik.
...
* Es reicht zur Erfüllung dieses Prüfschritts nicht aus, wenn sich der Ton über Tastaturbefehle (etwa die _Escape_-Taste) abschalten lässt.
...
* Automatisch abgespielter Ton enthält von Beginn an längere Sprachpassagen oder andere laute und störende Ton-Ereignisse.
...
This difficulty is exacerbated when the screen reader's speech output is software based (as most are today) and is controlled via the same volume control as the sound.
...
Note: Playing audio automatically when landing on a page may affect a screen reader user's ability to find the mechanism to stop it because they navigate by listening and automatically started sounds might interfere with that navigation.
